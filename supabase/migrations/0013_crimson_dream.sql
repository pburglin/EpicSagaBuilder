...
CREATE OR REPLACE FUNCTION start_story_round(story_id_val uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT COALESCE(MAX(sr.round_number), 0) + 1
  INTO current_round_number
  FROM story_rounds sr
  WHERE sr.story_id = story_id_val;
  
  -- Create new round
  INSERT INTO story_rounds (story_id, round_number)
  VALUES (story_id_val, current_round_number)
...
DECLARE
  ...
  story_id_val uuid;
BEGIN
  -- Get round info and story_id
  SELECT * INTO round_record
  FROM story_rounds
  WHERE id = round_id;
  
  story_id_val := round_record.story_id;
  
  -- Get number of active characters
  SELECT COUNT(*)
  INTO active_characters
  FROM characters c
  WHERE c.story_id = story_id_val
  AND c.status = 'active';
